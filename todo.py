...
def display_tasks(task_list, show_completed=False):
 ...
 print(f"{idx}. [{status}] {task['description']} - Due Date:
{task['due_date']} - Category: {task['category']}")
...
format.")
 ...
 task_category = input("Enter task category (e.g., Work, Personal,
Urgent): ")

 # Add task and save
 add_new_task(task_list, task_desc, task_due_date,
task_category)
 save_tasks(task_list, file_path)

 elif choice == '2':
 # Display tasks with option to show completed ones
 show_completed = input("Show completed tasks? (y/n):
").lower() == 'y'
 ...
 display_tasks(task_list)
 try:
 task_idx = int(input("Enter the task index to mark as
completed: "))
